...
Outside our app module, we have a few build files. These are generated by the build tool which Android Studio uses, Gradle. For now, think of Gradle as the thing which compiles all your java classes, xml code etc and makes the APK which is installed on to your device. <br>Let's have a look at the build.gradle file. You'll see two files, one at the project level (Labelled 'Project: AP_W1'), and one at the module level (Labelled 'Module: app'). We'll be editing the module level build.gradle file most of the time. You'll see a bunch of confusing stuff here, but you might understand some parameters like the minSdkVersion which is set to 21. SDK 21 corresponds to Android 5.0 Lollipop. This means only apps running Android 5.0 & above can use this app.<br>
